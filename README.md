...
1. リポジトリをクローン:
   ```bash
   git clone https://github.com/your-username/caffei-native.git
   cd caffei-native
   ```
...
監視対象のアプリリストや動作ログなどの設定は、ユーザーのアプリケーション設定ディレクトリ（`~/Library/Application Support/com.your-username.caffei-native/config.json`）に永続化されます。
...
GitHub Actionsが構成されており、新しいタグ（例: `v0.1.0`）をプッシュするか手動でワークフローをトリガーすることで、macOS向けの `.app` とインストーラ (`.dmg`) が自動的にビルドされ、GitHub Releasesにアップロードされます。
